...
char g_szAppName[] = "MultiEditeur";
char g_szChild[] = "MultiEditeurMDIChild";
HINSTANCE hInst;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpszCmdParam, int nCmdShow) {
	...
	WndClassEx.hIcon           = LoadIcon(hInst, (LPCSTR)IDI_ICON1);
	WndClassEx.hCursor         = LoadCursor(hInst,(LPCSTR) IDC_CURSOR1);
	WndClassEx.hbrBackground   = (HBRUSH)(COLOR_3DSHADOW+1);
	WndClassEx.lpszMenuName    = (LPSTR)IDC_COMMIS2ENTREPOT2022;
	WndClassEx.lpszClassName   = g_szAppName;
	WndClassEx.hIconSm         = LoadIcon(hInst, (LPCSTR)IDI_ICON2);
	if(!RegisterClassEx(&WndClassEx)) {		MessageBox(0, "Could Not Register Window", "MultiEditeur",MB_ICONEXCLAMATION | MB_OK);	return -1;	}
	WndClassEx.lpfnWndProc     = MDIChildWndProc;
	WndClassEx.lpszMenuName    = NULL;
	WndClassEx.lpszClassName   = g_szChild;
	WndClassEx.hbrBackground   = (HBRUSH)(COLOR_INFOBK+1);
	if(!RegisterClassEx(&WndClassEx)) {
		MessageBox(0, "Could Not Register Child Window", "MultiEditeur",
		MB_ICONEXCLAMATION | MB_OK);
		return -1;
	}
#ifdef _WIN64
#define titre "Projet MultiEditeur Edition 64bits"
#else
#define titre "Projet MultiEditeur Edition 32bits"
#endif
	hMain = CreateWindowEx(WS_EX_APPWINDOW,g_szAppName,titre,WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,CW_USEDEFAULT,CW_USEDEFAULT,CW_USEDEFAULT,CW_USEDEFAULT,0, 0, hInstance, NULL);
	if (hMain == NULL){
		MessageBox(0, "No Window", "MultiEditeur", MB_ICONEXCLAMATION | MB_OK);
		return -1;
	}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	switch(Message) {
		case WM_CREATE: {
			...
			g_hMDIClient = CreateWindowEx(WS_EX_DLGMODALFRAME|WS_EX_WINDOWEDGE|WS_EX_CLIENTEDGE, "mdiclient", NULL,WS_CHILD | WS_CLIPCHILDREN | WS_VSCROLL | WS_HSCROLL,CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,hwnd, (HMENU)ID_MDI_CLIENT, hInst, (LPVOID)&ccs);
			...
			SendMessage(g_hStatusBar, SB_SETTEXT, 0, (LPARAM)"MultiEditeur 2021");
			g_hToolBar = CreateWindowEx(0, TOOLBARCLASSNAME, NULL,WS_CHILD | WS_VISIBLE, 0, 0, 0, 0,hwnd, (HMENU)ID_TOOLBAR, hInst, NULL);
			...
			tbb[3].idCommand = CM_FILE_SAVEAS;			
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_NEW: {
					...
					if(!hChild) {MessageBox(hwnd, "MDI Child creation failed.", "MultiEditeur",MB_ICONEXCLAMATION | MB_OK);	}
					SetClassLongA(hChild, -14, (long)LoadIcon(hInst, (LPCSTR)IDI_ICON3));
					break;
				}
				case CM_FILE_OPEN: {
				...
				case CM_WINDOW_TILEHORZ:	PostMessage(g_hMDIClient, WM_MDITILE, MDITILE_HORIZONTAL, 0);break;
				case CM_WINDOW_TILEVERT:	PostMessage(g_hMDIClient, WM_MDITILE, MDITILE_VERTICAL, 0);	 break;
				case CM_WINDOW_CASCADE:		PostMessage(g_hMDIClient, WM_MDICASCADE, 0, 0);				break;
				case CM_WINDOW_ARRANGE:		PostMessage(g_hMDIClient, WM_MDIICONARRANGE, 0, 0);			break;
				default: {
...
LRESULT CALLBACK MDIChildWndProc(HWND hwnd, UINT Message, WPARAM wParam,LPARAM lParam) {
	switch(Message) {
		case WM_CREATE: {
			char szFileName[MAX_PATH];
			HWND hEdit;			
			hEdit = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", "", WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | ES_MULTILINE | ES_WANTRETURN,CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,	hwnd, (HMENU)IDC_CHILD_EDIT, hInst, NULL);			
			SendMessage(hEdit, WM_SETFONT,
			(WPARAM)GetStockObject(0x03), MAKELPARAM(TRUE, 0));
						GetWindowText(hwnd, szFileName, MAX_PATH);
			if(*szFileName != '[') {
				if(!LoadFile(hEdit, szFileName)) {
					MessageBox(hwnd, "Couldn't Load File.", "MultiEditeur",MB_OK | MB_ICONEXCLAMATION);
					return -1; //cancel window creation
				}
			}
			break;
		}
	case WM_CTLCOLOREDIT:SetTextColor((HDC)wParam, RGB(0,192,  0)); SetBkMode((HDC)wParam, TRANSPARENT); return (LONG)CreateSolidBrush(RGB(0,0,0));
		case WM_SIZE:
		...
		case WM_MDIACTIVATE: {
			...
			hMenu = GetMenu(hMain);
			if(hwnd == (HWND)lParam){ EnableFlag = TRUE;} else{EnableFlag = FALSE; 	}
			...
			GetWindowText(hwnd, szFileName, MAX_PATH);
			SendMessage(g_hStatusBar, SB_SETTEXT, 2, (LPARAM)szFileName );
			break;
		}
		case WM_SETFOCUS:
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
		case ID_FICHIER_SUPPRIMER: {char tmp[256]; GetWindowText(hwnd,tmp,sizeof(tmp)); if(MessageBox(0,tmp,"Suppression",MB_YESNO)==0x06){DeleteFile(tmp);}break;}
				case CM_FILE_SAVE: {
					char szFileName[MAX_PATH];					
					GetWindowText(hwnd, szFileName, MAX_PATH);
					if(*szFileName != '[') {
						if(!SaveFile(GetDlgItem(hwnd, IDC_CHILD_EDIT), szFileName)) {
							MessageBox(hwnd, "Couldn't Save File.", "MultiEditeur",MB_OK | MB_ICONEXCLAMATION);
							return 0;
						}
					} else {
				...
				case CM_EDIT_UNDO:SendDlgItemMessage(hwnd, IDC_CHILD_EDIT, EM_UNDO, 0, 0);	break;
				case CM_EDIT_CUT:SendDlgItemMessage(hwnd, IDC_CHILD_EDIT, WM_CUT, 0, 0);	break;
				case CM_EDIT_COPY:SendDlgItemMessage(hwnd, IDC_CHILD_EDIT, WM_COPY, 0, 0);	break;
				case CM_EDIT_PASTE:SendDlgItemMessage(hwnd, IDC_CHILD_EDIT, WM_PASTE, 0, 0);break;
			}
			return 0;
		}
	}
