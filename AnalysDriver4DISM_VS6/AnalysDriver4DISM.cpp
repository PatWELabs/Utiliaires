...
int main()
{
   ...
   strncat(InfoDir, TEXT("\\Inf"),0x100);
length_of_arg=strlen(InfoDir);
   if (length_of_arg > (MAX_PATH - 3)){_tprintf(TEXT("\nChemin trop long.\n"));return (-1);}
   _tprintf(TEXT("\nAnalyse de fichiers de configuration Windows %s\n\n"), InfoDir);
   strncpy(szDir, InfoDir, MAX_PATH);
   strncat(szDir, TEXT("\\*"), MAX_PATH);
...
void DisplayErrorBox(LPTSTR lpszFunction) { 
    ...
    sprintf((LPTSTR)lpDisplayBuf, TEXT("%s failed with error %d: %s"),LocalSize(lpDisplayBuf) / sizeof(TCHAR),lpszFunction, dw, lpMsgBuf); 
