...
int setup_for_ping(char* host, int ttl, SOCKET& sd, sockaddr_in& dest){
    ...
    if (addr != INADDR_NONE) {dest.sin_addr.s_addr = addr;        dest.sin_family = AF_INET;
        ...
        else {   cerr << "Failed to resolve " << host << endl;            return -1;        }
    }
...
void init_ping_packet(ICMPHeader* icmp_hdr, int packet_size, int seq_no){
    ...
    while (bytes_left > 0) {    memcpy(datapart, &deadmeat, min(int(sizeof(deadmeat)),  bytes_left));        bytes_left -= sizeof(deadmeat);        datapart += sizeof(deadmeat);    }
...
int send_ping(SOCKET sd, const sockaddr_in& dest, ICMPHeader* send_buf,int packet_size){
    cout << "Sending " << packet_size << " bytes to " <<             inet_ntoa(dest.sin_addr) << "..." << flush;    int bwrote = sendto(sd, (char*)send_buf, packet_size, 0,             (sockaddr*)&dest, sizeof(dest));
...
int recv_ping(SOCKET sd, sockaddr_in& source, IPHeader* recv_buf, int packet_size){
	...
	if (bread == SOCKET_ERROR) {        cerr << "read failed: ";        if (WSAGetLastError() == WSAEMSGSIZE) {
...
int decode_reply(IPHeader* reply, int bytes, sockaddr_in* from) 
{
    ...
    if (bytes < header_len + ICMP_MIN) {        cerr << "too few bytes from " << inet_ntoa(from->sin_addr) <<  endl;        return -1;    }
    else if (icmphdr->type != ICMP_ECHO_REPLY) {
        if (icmphdr->type != ICMP_TTL_EXPIRE) {            if (icmphdr->type == ICMP_DEST_UNREACH) {                cerr << "Destination unreachable" << endl;            }
            else {                cerr << "Unknown ICMP packet type " << int(icmphdr->type) <<                        " received" << endl;}
            return -1;
        }
    }
    ...
    if (icmphdr->type == ICMP_TTL_EXPIRE) {        cout << "TTL expired." << endl;    }
    else {        cout << nHops << " hop" << (nHops == 1 ? "" : "s");        cout << ", time: " << (GetTickCount() - icmphdr->timestamp) <<                " ms." << endl;    }
